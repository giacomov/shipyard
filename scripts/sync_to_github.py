#!/usr/bin/env python3
"""Mirror a tasks.json task list to GitHub Issues with blocked-by relationships.

Port of invaders/scripts/sync-tasks-to-github.mjs.

Usage:
    python scripts/sync_to_github.py --input tasks.json
    python scripts/sync_to_github.py --input tasks.json --repo owner/repo --dry-run
"""
...
def sync(data: dict, repo: str, dry_run: bool) -> int:
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Sync tasks.json to GitHub Issues")
    parser.add_argument("--input", "-i", type=Path, help="Input tasks.json (default: stdin)")
    parser.add_argument("--repo", help="Target repo as owner/repo (default: auto-detect)")
    ...
    exit_code = sync(data, repo, args.dry_run)
    sys.exit(exit_code)
